...
/// Track the last-known screen recording status so we can detect revocations
/// when the app resumes from the background.
#[cfg(target_os = "macos")]
static SR_WAS_GRANTED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// Check screen recording status on resume. If it was previously granted but
/// is now denied, emit a `screen-recording-lost` event to all webviews.
#[cfg(target_os = "macos")]
fn check_sr_on_resume<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    let current = crate::macos::permissions::check_screen_recording() == "granted";
    let was_granted = SR_WAS_GRANTED.load(std::sync::atomic::Ordering::Relaxed);

    if was_granted && !current {
        let _ = app.emit("screen-recording-lost", ());
    }

    SR_WAS_GRANTED.store(current, std::sync::atomic::Ordering::Relaxed);
}

/// Initialize screen recording tracking — snapshot the current status.
#[cfg(target_os = "macos")]
fn init_sr_tracking() {
    let granted = crate::macos::permissions::check_screen_recording() == "granted";
    SR_WAS_GRANTED.store(granted, std::sync::atomic::Ordering::Relaxed);
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Snapshot initial screen recording status for revocation detection.
            #[cfg(target_os = "macos")]
            init_sr_tracking();
        ...
        .run(|app, event| {
            match event {
                ...
                // Re-check screen recording when the app returns to the foreground.
                RunEvent::Resumed => {
                    #[cfg(target_os = "macos")]
                    check_sr_on_resume(app);
                }
                _ => {}
            }
        });
